...
@interface PrefixViewController () <CLLocationManagerDelegate>
...
- (id)init
{
    ...
    if (self) {
        ...
        self.prefixNumber = [self getPrefixByCityName:countryCode];
         self.Country_num = self.prefixDict[countryCode];
...
-(NSString *)getPrefixByCityName:(NSString *)cityName{
    ...
    NSString * prefix= self.prefixDict[cityName];
    return [NSString stringWithFormat:@"+%@", prefix];;
}

-(void)fillDictionary{
    self.prefixDict = [NSDictionary dictionaryWithObjectsAndKeys:
